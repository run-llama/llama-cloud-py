...
from ._exceptions import APIStatusError, LlamacloudProdError
...
class LlamacloudProd(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous LlamacloudProd client instance.

        This automatically infers the `api_key` argument from the `LLAMACLOUD_PROD_API_KEY` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("LLAMACLOUD_PROD_API_KEY")
        if api_key is None:
            raise LlamacloudProdError(
                "The api_key client option must be set either by passing api_key to the client or by setting the LLAMACLOUD_PROD_API_KEY environment variable"
            )
        self.api_key = api_key

        if base_url is None:
            base_url = os.environ.get("LLAMACLOUD_PROD_BASE_URL")
        if base_url is None:
...
class AsyncLlamacloudProd(AsyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async AsyncLlamacloudProd client instance.
...
class LlamacloudProdWithRawResponse:
...
AsyncClient = AsyncLlamacloudProd
